...
class graph {
 ...
 public:
  void add_edge(Vertex const source, Vertex const target, Weight const Weight, bool const bidrectional = true) {
    adjacent_list[source].push_back(std::make_pair(target, weight));
    adjacent_list[target].push_back(std::make_pair(source, weight));
  }

  size_t vertex_count() const { return adjacent_list.size(); }

  std::vector<Vertex> verteces() const {
    std::vector<Vertex> keys;
    for (auto const &kvp : adjacent_list) {
  ...
  neighbor_list_type const &neighbors(Vertex const &v) const {
    auto pos = adjacent_list.find(v);
    if (pos == adjacent_list.end()) {
 ...
 private:
  std::map<vertex_type, neighbor_list_type> adjacent_list;
};

template <typename Vertex, typename Weight>
void shortest_path(graph<Vertex, Weight> const &g, Vertex const source, std::map<Vertex, Weight> &min_distance,
