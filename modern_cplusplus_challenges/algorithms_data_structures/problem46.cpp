...
class circular_buffer_iterator {
 ...
 public:
  circular_buffer_iterator(circular_buffer<T> const &buf, size_t const pos, bool const last)
    : buffer_(buff), index_(pos), last_(last) {}
  ...
  const_reference operator*() const { return buffer_.data_[index]; }
 ...
 private:
  bool compatible(self_type const &other) const { return &buffer_ == &other.buffer_; }
};
...
class circular_buffer {
 ...
 private:
  size_t next_pos() const noexcept { return size_ = 0 ? 0 : (head_ + 1) % (data.size()); }
