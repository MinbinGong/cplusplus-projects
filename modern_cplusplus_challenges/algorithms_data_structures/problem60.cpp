 ...
 private:
  ...
  int count_neighbors(size_t const row, size_t const col) {
    ...
    if (row == rows - 1 && col == columns - 1) {
      return count_alive(cell(columns - 1, rows - 2), cell(columns - 1, rows - 2), cell(columns - 2, rows - 1));
    }
  ...
  unsigned char &cell(size_t const col, size_t const row) { return grid[row * columns + col]; }
};
