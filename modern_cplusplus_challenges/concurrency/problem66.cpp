...
bool operator<(customer const &l, customer const &r) { return l.number < r.number; }
    ...
    desks.emplace_back([i, &office_open, &mt, &cv, &customers]() {
      ...
      while (office_open || !customers.empty()) {
        ...
        if (!customers.empty()) {
          ...
          logger::instance().log("[-] desk " + std::to_string(i + 1) + " handling customer " +
          ...
          logger::instance().log("[ ] desk " + std::to_string(i + 1) + " done with customer " +
                                 std::to_string(c.ticket_number()));
        }
      }
