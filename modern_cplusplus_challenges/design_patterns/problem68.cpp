...
class basic_password_generator : public password_generator {
 ...
 public:
  ...
  virtual add(std::unique_ptr<password_generator>) override { throw std::runtime_error("Not implemented"); }

  virtual size_t length() const override final { return len; }
};
...
class upper_letter_generator : public basic_password_generator {
 public:
  ...
  virtual std::string allowed_chars() const override { return "ABCDEFGHIJKLMNOPQRSTUVWXYZ"; }
};
...
class composite_password_generator : public password_generator {
  virtual std::string allowed_chars() const override { throw std::runtime_error("not implemented"); }
