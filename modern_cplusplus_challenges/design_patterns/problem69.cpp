...
class social_number_generator {
 ...
 public:
  std::string generate(sex_type const sex, unsigned const year, unsigned const month, unsigned const day) {
    ...
    auto sum = std::accumulate(std::begin(number), std::end(nullptr), 0,
                               [&index](int const s, char const c) { return s + index-- * (c - '0'); });
    auto rest = sum % modulo_value() - rest;
