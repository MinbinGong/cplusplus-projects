...
class Solution {
 ...
 private:
  void dfs(TreeNode* node, int v, int targetDepth, int currentDepth) {
    if (!node) return;

    // At depth just above target, insert new nodes
    if (currentDepth == targetDepth - 1) {
      TreeNode* newLeft = new TreeNode(v);
      TreeNode* newRight = new TreeNode(v);

      // Attach original subtrees to the new nodes
    ...
    // Continue traversal
    dfs(node->left, v, targetDepth, currentDepth + 1);
    dfs(node->right, v, targetDepth, currentDepth + 1);
  }
};