...
int rank(int const prefix, int const max_val) {
  int r = 1;
  int num_digits = 0;
  ...
  for (int p = prefix; p; p /= 10, ++num_digits)
    for (int d = 1; d <= p % 10; ++d) r += numPrefixes(p - d, max_val);
  r += num_digits - 1;
  return r;
}
int numPrefixes(int const prefix, int const max_val) {
  ...
  for (unsigned long long val = prefix; val <= max_val; p *= 10, val *= 10) cnt += min(p, max_val - val + 1);
  return cnt;
}
