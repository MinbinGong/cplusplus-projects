...
bool jump(unordered_map<int, unordered_map<int, int>> &mp, unordered_set<int> &stset, int goalst, int pos, int k)
{
    ...
    if (stset.count(pos) == 0 || pos > goalst)
        return false;

    if (goalst == pos)
        return true;

    if (mp.count(pos) && mp[pos].count(k))
        return mp[pos][k];

    bool success = jump(mp, stset, goalst, pos + k - 1, k - 1) || jump(mp, stset, goalst, pos + k, k) || jump(mp, stset, goalst, pos + k + 1, k + 1);
