...
class Twitter {
 ...
 public:
  ...
  vector<int> getNewsFeed(int userId) {
    ...
    if (idx[userId] >= 0) pq.push({posts[userId].back().second, userId});

    for (auto x : followees[userId]) {
      if (!posts.count(x)) continue;
  ...
  void unfollow(int followerId, int followeeId) {
    if (followees[followerId].count(followeeId)) followees[followerId].erase(followeeId);
  }
};
