 ...
 * There are no users with a UAM of 3 or 4. Hence, answer[3] and answer[4] are 0.
 * 
 * Constraints:
 * 
 * 1 <= logs.length <= 104
...
public:
    std::vector<int> findingUsersActiveMinutes(std::vector<std::vector<int>>& logs, int k) {
        ...
        for (const auto& entry : userMinutes) {
            ...
            // 根据题意，activeMinutes 一定在 [1, k] 范围内
            ++answer[activeMinutes - 1];
        }
