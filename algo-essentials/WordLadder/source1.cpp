...
std::unordered_map<std::string, std::unordered_set<std::string>> build_graph(
      ...
      for (char c = 'a'; c <= 'z'; c++) {
        ...
        if (dict.find(new_word) != dict.end()) {
          ...
          if (iter != adjacent_list.end()) {
            ...
            adjacent_list.insert(
              std::pair<std::string, std::unordered_set<std::string>>(word, std::unordered_set<std::string>()));
...
std::vector<std::vector<std::string>> find_ladders(const std::string& start, const std::string& end,
                                                   const std::unordered_set<std::string>& dict) {
  ...
  auto state_is_valid = [&](const std::string& s) { return dict.find(s) != dict.end() || s == end; };
  auto state_is_target = [&](const std::string& s) { return s == end; };
