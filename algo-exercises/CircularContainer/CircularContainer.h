...
class CircularContainer {
 ...
 private:
  ...
  size_t next_pos() const noexcept { return mSize == 0 ? 0 : (mHead + 1) % mData.size(); }
  size_t first_pos() const noexcept { return mSize == 0 ? 0 : (mHead + mData.size() - mSize + 1) % mData.size(); }
...
class CircularContainerIterator {
 ...
 public:
  CircularContainerIterator(CircularContainer<T> const& buf, size_t const pos, bool const last)
    : mBuffer(buf->mSize), mIndex(pos), mLast(last) {}
  ...
  const_reference operator*() const { return mBuffer.mData[mIndex]; }

  const_reference operator->() const { return mBuffer.mData[mIndex]; }

 private:
  bool compatible(self_type const& other) const { return &mBuffer == &other.mBuffer; }
